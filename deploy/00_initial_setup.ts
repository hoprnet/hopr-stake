...
const main: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  try {
    fs.unlinkSync(`deployments/${network.name}/contractAddress.ts`);
  } catch (e) {
    console.log("First time deploying, skipping unlinking...");
  }
  ...
  saveFrontendFiles(multicallContract, "MulticallContract", network.name);
  console.log("Multicall deployed to:", multicallContract.address);

  // We get the contract to deploy
  const [deployer, admin, alice]: SignerWithAddress[] =
    await ethers.getSigners();
  const HoprBoostContractFactory: HoprBoost__factory =
    (await ethers.getContractFactory(
      HOPR_BOOST_CONTRACT
    )) as HoprBoost__factory;
  const boostContract = await HoprBoostContractFactory.deploy(
  ...
  console.log(
    `We minted ${await boostContract.totalSupply()} NFTs, to ${aliceAddress} which has a nominator boost of ${nominator} by ${deadline}`
  );
  saveFrontendFiles(boostContract, HOPR_BOOST_CONTRACT, network.name);
  ...
  saveFrontendFiles(erc677, "xHOPR", network.name);
  const erc777 = await deployContract(deployer, "ERC777Mock");
  await erc777.deployed();
  saveFrontendFiles(erc777, "wxHOPR", network.name);

  const adminAddress = await admin.getAddress();

  // We deploy the staking contract with a given Boost smart contract using specific (wx)HOPR and (x)HOPR token
  const HoprStakingContractFactory: HoprStake__factory =
    (await ethers.getContractFactory(
      HOPR_STAKE_CONTRACT
    )) as HoprStake__factory;
  const stakeContract = await HoprStakingContractFactory.deploy(
  ...
  await erc677.batchMintInternal(
    [aliceAddress],
    utils.parseUnits("10000", "ether")
  );
  // Admin now has 5 million wxHOPR
  await erc777.mintInternal(
    adminAddress,
  ...
  await registry.getInterfaceImplementer(
    ...
    utils.keccak256(utils.toUtf8Bytes("ERC777TokensRecipient"))
  );
  ...
  await erc777
    ...
    .send(
      stakeContract.address,
      utils.parseUnits("1000000", "ether"),
      constants.HashZero
    );
  ...
  console.log(
    `${HOPR_STAKE_CONTRACT} was deployed with ${await stakeContract.LOCK_TOKEN()} as LOCK_TOKEN`
  );
  console.log(
    `${HOPR_STAKE_CONTRACT} was deployed with ${await stakeContract.REWARD_TOKEN()} as REWARD_TOKEN`
  );
  // Showcasing the initial state of the staking contract and admin account
  console.log(
    `Sending ${utils.parseUnits(
      "1000000",
      "ether"
    )} from ${adminAddress} (admin) to ${
      stakeContract.address
    } (staking contract)`
  );
  console.log(
    `${HOPR_STAKE_CONTRACT} has ${await stakeContract.availableReward()} available rewards`
  );
  console.log(
    `${adminAddress} (admin) has now ${await erc777.balanceOf(adminAddress)}`
  );
  saveFrontendFiles(stakeContract, HOPR_STAKE_CONTRACT, network.name);
...
function saveFrontendFiles(
  ...
  fs.appendFileSync(
    `deployments/${network}/contractAddress.ts`,
