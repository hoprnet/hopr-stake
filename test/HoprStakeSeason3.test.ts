...
(whitehatTestOnly ? describe.skip : describe)('HoprStakeSeason3', function () {
    let deployer: Signer;
    let admin: Signer;
    ...
    const PROGRAM_S3_START = 1650974400; // Apr 26th 2022 14:00 CET.
    const PROGRAM_S3_END = 1658836800; // July 26th 2022 14:00 CET.
    const BADGES = [
        {
            type: "demo",
            rank: "demo",
            deadline: PROGRAM_S3_START,
    ...
    const reset = async () => {
        ...
        stakeContract = await deployContract4(deployer, "HoprStakeSeason3", nftContract.address, adminAddress, erc677.address, erc777.address);
    ...
    describe('unit tests', function () {
        ...
        describe('Can redeem allowed NFT', function () {
            ...
            it(`fail to redeem nfts nr ${hodlrNftTokenId}`, async () => {
                await expect(
                    nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stakeContract.address, hodlrNftTokenId)
    ...
    describe('After PROGRAM_S3_END', function () {
        ...
        it('succeeds in advancing block to PROGRAM_S3_END + 1', async function () {
            await advanceTimeForNextBlock(PROGRAM_S3_END + 1);
            const [blockTime, _] = await latestBlockTime();
            expect(blockTime.toString()).to.equal((PROGRAM_S3_END + 1).toString()); 
        });
        ...
        it('cannot redeem NFT', async () => {
            ...
            expectRevert(nftContract.connect(participants[1]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[1], stakeContract.address, 1), 'HoprStake: Program ended, cannot redeem boosts.');
        }); 
        it('can unlock', async () => {
