...
(TEST_WHITEHAT_ONLY.toLowerCase() == 'true' ? describe : describe.skip)('HoprWhitehat', function () {
    ...
    const reset = async () => {
        ...
        xHopr = await deployContract(deployer, "ERC677Mock");
        wxHopr = await deployContract(deployer, "ERC777Mock");
        // create NFT and stake contract
        nftContract = await deployContract2(deployer, "HoprBoost", adminAddress, "");
        stakeContract = await deployContract4(deployer, "HoprStake", nftContract.address, adminAddress, xHopr.address, wxHopr.address);

        // create whitehat contract
        whitehatContract = await deployContract4(deployer, "HoprWhitehat", adminAddress, nftContract.address, stakeContract.address, xHopr.address, wxHopr.address)

        // airdrop some NFTs of type (0,1 and 2) to staker[0]
        await Promise.all(BADGES.map(badge => nftContract.connect(admin).mint(stakerAddresses[0], badge.type, badge.rank, badge.nominator, badge.deadline)))
        // airdrop some NFTs of type (0,1 and 2) to staker[2]
        await Promise.all(BADGES.map(badge => nftContract.connect(admin).mint(stakerAddresses[2], badge.type, badge.rank, badge.nominator, badge.deadline)))
    ...
    describe('unit tests', function () {
        ...
        it('participants have received ERC721', async function () {
            expect((await nftContract.tokenOfOwnerByIndex(stakerAddresses[0], 0)).toString()).to.equal(constants.Zero.toString());
        });
        ...
        describe('LOCK_TOKEN and other ERC677 token', () => {
            ...
            it('can receive random 677 with `transferAndCall()`', async () => {
                await expect(randomERC677.connect(stakers[0]).transferAndCall(whitehatContract.address, utils.parseUnits('1000', 'ether'), '0x'))
                .to.emit(whitehatContract, 'Received677')
                .withArgs(randomERC677.address, stakerAddresses[0], utils.parseUnits('1000', 'ether').toString())
                expect((await randomERC677.balanceOf(stakerAddresses[0])).toString()).to.equal(utils.parseUnits('9000', 'ether').toString());
            }); 
            ...
            it('can receive xHopr with `transferAndCall()`', async () => {
                await expect(xHopr.connect(stakers[0]).transferAndCall(whitehatContract.address, utils.parseUnits('1000', 'ether'), '0x'))
                .to.emit(whitehatContract, 'Received677')
                .withArgs(xHopr.address, stakerAddresses[0], utils.parseUnits('1000', 'ether').toString())
                expect((await xHopr.balanceOf(stakerAddresses[0])).toString()).to.equal(utils.parseUnits('9000', 'ether').toString());
            });
            it('does update rescuedXHoprAmount for random677', async () => {
                expect((await whitehatContract.rescuedXHoprAmount()).toString()).to.equal(utils.parseUnits('1000', 'ether').toString()); // rescuedXHoprAmount should be updated
            });
        });
        ...
        describe('REWARD_TOKEN and other ERC777 token', () => {
            ...
            describe('global switch is on', () => {
                ...
                it('can not receive random 777 with `send()` when the globalSwitch is on', async () => {
                    expectRevert(randomERC777.connect(stakers[0]).send(whitehatContract.address, constants.One, '0x'),  "can only be called from wxHOPR");
                }); 
    ...
    describe('Integration: When s1 PROGRAM_END', function () {
        ...
        before(async function () {
            ...
            await Promise.all(stakers.map((staker) => xHopr.connect(staker).transferAndCall(stakeContract.address, utils.parseUnits(INITIAL_STAKE, 'ether'), '0x')))
            // provide 100k reward
            await wxHopr.connect(admin).send(stakeContract.address, utils.parseUnits('100000', 'ether'), '0x');
        })
        it('succeeds in advancing block to PROGRAM_END', async function () {
            // avance to end of program
            await advanceTimeForNextBlock(PROGRAM_END);
            const [blockTime, _] = await latestBlockTime();
            expect((blockTime).toString()).to.equal(PROGRAM_END.toString()); 
        });
        describe('After v1 PROGRAM_END', function () {
            ...
            before(async function () {
                ...
                await Promise.all(deadLocked.map(stakerIndex => stakeContract.connect(stakers[stakerIndex]).claimRewards(stakerAddresses[stakerIndex])))
            ...
            deadLocked.map((lockedIndex) => {
                it(`cannnot unlock for deadlocked staker ${lockedIndex}`, async function () {
                    expectRevert(stakeContract.connect(stakers[lockedIndex]).unlock(stakerAddresses[lockedIndex]),  "HoprStake: Nothing to claim");
                });
            })
            // canLock.map(stakerIndex => {
            //     it(`can unlock for canLock staker ${stakerIndex}`, async function () {
            //         await expect(stakeContract.connect(stakers[stakerIndex]).unlock(stakerAddresses[stakerIndex]))
            //         .to.emit(stakeContract, 'Released')
            //         .withArgs(stakerAddresses[stakerIndex], utils.parseUnits('1000', 'ether'), '0')
            //     });
            // })
            describe('Use WhiteHat contract', function () {
                ...
                before(async function () {
                    ...
                    await Promise.all(canLock.map(stakerIndex => erc1820.connect(stakers[stakerIndex]).setInterfaceImplementer(stakerAddresses[stakerIndex], interfaceHash, whitehatContract.address)))
